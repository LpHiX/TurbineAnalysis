...
startTorque = zeros([1, 19]);
for i = 1:19
    airfoil_profile(1:i) = 2;
    airfoil_profile(i+1:20) = 1;
    ...
    startTorque(i) = torque;
    fprintf("Starting Torque is %8.3f \n", torque);
end
plot(R0 + (R - R0) * [1:19] / 19, startTorque)
xlabel("Radius at which airfoil switches from NACA23024 to SG6043")
ylabel("Starting Torque");
% 
% for DesignTSR = 3:8
%     [d_chord, d_twist] = designblade(B,R,R0,N_sections,DesignTSR,airfoil_profile,polars);
% 
%     TSR_list = 0:0.1:10;
...
%     end
%     plot(TSR_list(CPlist>0),CPlist(CPlist>0), 'DisplayName',(num2str(DesignTSR)));
%     hold on
%     DesignTSR
% end
% 
% leg = legend;
% title(leg, "Design TSR");
% xlabel("TSR")
% ylabel("Cp")